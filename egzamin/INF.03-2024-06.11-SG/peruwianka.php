...
<html lang="pl">
	...
	<body>
		...
		<main>
			...
			<!-- skrypt 2 -->
			<?php
				$res = mysqli_query($conn, "SELECT DISTINCT s.data_ur, s.miot, r.rasa FROM rasy r JOIN swinki s ON r.id = s.rasy_id WHERE r.id = 1;");
				while($row = mysqli_fetch_row($res)) {
					echo "
						<h2>Rasa: {$row[2]}</h2>
			...
			<!-- skrypt 3 -->
			<?php
				$res = mysqli_query($conn, "SELECT DISTINCT s.imie, s.cena, s.opis FROM swinki s WHERE s.rasy_id = 1;");
				while($row = mysqli_fetch_row($res)) {
					echo "
